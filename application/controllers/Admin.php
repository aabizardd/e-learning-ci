...
class admin extends CI_Controller {
    ...
    public function updateKelasSiswa($idsiswa,$idkelassiswa = null)
    {
        $existMapel = $this->Admin_model->view_where('el_kelas_siswa',array('siswa_id'=>$idsiswa,'kelas_id'=>$this->input->post('kelas_id')))->result_array();
        $this->Admin_model->update(array('aktif'=>0),array('id'=>$idkelassiswa),'el_kelas_siswa');

        print_r($existMapel);
        if($existMapel[0]['kelas_id'] == $this->input->post('kelas_id')) {
            $this->Admin_model->update(array('aktif'=>1),array('id'=>$existMapel[0]['id']),'el_kelas_siswa');
        } else {
    ...
    public function simpanPengajar()
    {
        ...
        $check_opsi = $this->input->post('opsi');
        if ($check_opsi == null) {
            $status_opsi = '0';
        } else {
            $status_opsi = $check_opsi;
        }
        $dataLogin=array(
            'username'=>$this->input->post('username'),
            'password'=>md5($this->input->post('password')),
            'pengajar_id'=>$return_id,
            'is_admin'=>$status_opsi
        );
        $this->Admin_model->insert($dataLogin,'el_login');
    ...
    {
        $this->Admin_model->delete(array('id'=>$id),'el_kelas');
