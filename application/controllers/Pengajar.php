...
class pengajar extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['pelajaran'] = $this->pengajar_model->getPelajaranPengajar($this->session->userdata('id'))->result();
        $data['tugas'] = $this->pengajar_model->getTugasPengajar($this->session->userdata('id'))->result();
        $data['materi'] = $this->pengajar_model->getMateriPengajar($this->session->userdata('id'))->result();
        $data['ujian'] = $this->pengajar_model->getUjianPengajar($this->session->userdata('id'))->result();
    ...
    public function penilaian($idtugas, $kelas, $mapelid)
    {
        ...
        $mapel = $this->pengajar_model->getMapel($mapelid)->row_array();
        $data['title'] = 'Nilai Tugas ' . $mapel['nama'];
    ...
    public function updateNilai($id, $idtugas, $kelas, $mapelid)
    {
        ...
        $data['idmapel'] = $mapelid;
        $data['idtugas'] = $idtugas;
        $data['title'] = $this->pengajar_model->getMapel($mapelid);
    ...
    {
        $mapel = $this->pengajar_model->getMapelPengajar($this->session->userdata('id'));

        if ($mapel['id_mapel'] == $id) {
            $data = array('id_mapel' => null);
    ...
    public function ujian()
    {
        ...
        $data['ujian'] = $this->pengajar_model->getUjianGuru($this->session->userdata('id'))->result();
    ...
    public function detailUjian($id)
    {
        ...
        $data['bank_soal'] = $this->db->get('el_banksoal')->result();
        //$data['id_soalnya']=$id;
        $this->load->view('part/header');
        $this->load->view('part/sidebarpengajar', $data);
    ...
    public function verifyUjian($status, $idUjian)
    {

        $this->db->update('el_ujian', ['is_verify' => $status], ['id' => $idUjian]);
        redirect('pengajar/detailUjian/' . $idUjian);
    ...
    public function hasilUjian($id)
    {
        ...
        $ujian = $this->pengajar_model->getUjian($this->session->userdata('id'), $id)->row_array();
        $data['title'] = 'Hasil Ujian ' . $ujian['kelas'] . ' | ' . $ujian['mapel'];
        // print_r($data);
        $this->load->view('part/header', $data);
        $this->load->view('part/sidebarpengajar', $data);
    ...
    public function simpanSoal($tipe, $id_ujian)
    {
        ...
        if ($tipe == 1) {
            ...
            if ($upload) {
                ...
                $values = array_merge($values, $datagambar);
            } else {
                $values = $values;
            }

            $insert_soal = $this->pengajar_model->insert($values, 'el_banksoal');
            // var_dump($insert_soal);die();
            $values_soal = [
                'id_banksoal' => $insert_soal,
                'pengajar_id' => $this->session->userdata('id'),
            ];

            $insert_id = $this->pengajar_model->insert($values_soal, 'el_soal');
    ...
    public function addfrombanksoal()
    {
        $id_banksoal = $this->input->post('soaldipilih');
        $id_ujian = $this->input->post('idujian');

        $id_banksoal = explode(',', $id_banksoal);

        // var_dump($id_banksoal);die();

        for ($i = 0; $i < count($id_banksoal); $i++) {
            $values_soal = [
                'id_banksoal' => $id_banksoal[$i],
                'pengajar_id' => $this->session->userdata('id'),
            ];
    ...
    public function editSoal($tipe)
    {
        ...
        if ($tipe == 1) {
            ...
            if ($_FILES['file-input']['name'] != "") {
                ...
                $datagambar = array(
                    'gambar' => $this->input->post('file_lama'),
                );
                $values = array_merge($values, $datagambar);
            }
            ...
            $this->db->update('el_banksoal', $values, $where);
            ...
            if ($_FILES['file-input']['name'] != "") {
                ...
                $datagambar = array(
                    'file_soal' => $this->input->post('file_lama'),
                );
                $values = array_merge($values, $datagambar);
            }
    ...
    public function nilaiEssay($id, $id_ujian)
    {
        ...
        $nilaiEssayConvert = ($nilaiPG * 70 / 100) + ($nilaiEssay * 30 / 100);

        $nilai_total = $nilaiEssayConvert;
    ...
    public function setabsensi($kelas, $mapel)
    {
        ...
        $data['jadwal'] = $this->pengajar_model->getJadwalAjar()->row();
        $this->load->view('part/header');
        $this->load->view('part/sidebarpengajar', $data);
        $this->load->view('pengajar/absen/setabsenkelas', $data);
        $this->load->view('part/footer');
    }
    ...
    public function absensi($idabsen)
    {
        ...
        $absen = $this->pengajar_model->getAbsenSiswa($idabsen)->row_array();

        $data['title'] = 'Laporan absensi kelas ' . $absen['nama_kelas'] . ' Pelajaran ' . $absen['nama_mapel'];
